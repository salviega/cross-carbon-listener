...
import carbonContractJson from '../../assets/deployments/optimism/Carbon.json' assert { type: 'json' }
import communicatorContractJson from '../../assets/deployments/optimism/Communicator.json' assert { type: 'json' }

dotenv.config()

const { OPTIMISM_GOERLI_RPC_URL, PRIVATE_KEY } = process.env

if (!OPTIMISM_GOERLI_RPC_URL || !PRIVATE_KEY) {
	throw new Error(
		'OPTIMISM_GOERLI_RPC_URL or PRIVATE_KEY not found in .env file'
	)
}

const scan = 'https://goerli-optimism.etherscan.io/tx'

const provider = new ethers.providers.JsonRpcProvider(OPTIMISM_GOERLI_RPC_URL)
